...
The **formSetAdPushInfo** handler in `/bin/httpd` is vulnerable to multiple stack overflows due to the absence of user input sanitization and bounds checking on parameters **mac** and **terminal** which can lead to corruption of data on the stack, hijacking of control flow, and DoS. The attack can be performed remotely.
