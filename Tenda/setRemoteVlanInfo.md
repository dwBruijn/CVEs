...
The **formSetVlanInfo** handler in `/bin/httpd` calls **formSetRemoteVlanInfo** (under certain conditions) which is vulnerable to a stack overflow due to the complete absence of user input sanitization and bounds checking on parameter **ID**, **vlan**, and **port** which can lead of corruption of data on the stack, hijacking of control flow, and DoS. The attack can be performed remotely.
...
Now we ca send a POST request to the `/goform/setVlanInfo` endpoint to trigger the stack overflow in `setRemoteVlanInfo`
