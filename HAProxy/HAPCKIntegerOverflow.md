...
Running HAProxy without UBSan (sanitization) will lead to out-of-bounds memory access downstream due to the corrupted **nlen** variable which is being used in later operations and thus can lead to DoS, heap corruption, information disclosure, or even possibly RCE using heap Feng Shui and controlled overflow in Huffman decoder.
